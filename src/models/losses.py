...
def l1_penalty(parameters) -> float:
    """Calculate the L2 penalty for the given parameters.
    ...
    Returns:
        float: The L2 penalty.
    """
    return torch.linalg.norm(parameters, ord=1)
...
def cosine_penalty(parameters) -> float:
    ...
    """
    return torchmetrics.functional.pairwise_cosine_similarity(parameters, reduction="sum").sum()
